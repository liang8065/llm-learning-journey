...
### 2.3 第一个LangChain程序：LCEL表达式链

> **重要**: LangChain 1.0+ 推荐使用 LCEL（LangChain Expression Language），用 `|` 操作符串联组件，替代旧版的 `LLMChain`。

```python
from langchain_core.prompts import PromptTemplate
from langchain_community.chat_models import ChatTongyi  # 通义千问（1.0+方式）

# 初始化LLM（1.0+ 使用 ChatModel）
llm = ChatTongyi(
    model="qwen-plus",
    dashscope_api_key="your-key"
)

# 创建Prompt模板（1.0+ 使用字典传参）
...
print(result.content)  # ChatModel返回AIMessage，用.content获取文本
```
...
| 旧版（已废弃） | 1.0+ 新版 |
|----------------|-----------|
| `from langchain.chains import LLMChain` | `chain = prompt \| llm`（LCEL） |
| `chain.run(...)` | `chain.invoke({...})` |
| `from langchain_community.llms import Tongyi` | `from langchain_community.chat_models import ChatTongyi` |
| `LLMChain(llm=llm, prompt=prompt)` | `prompt \| llm` |

### 2.4 Memory（记忆）让对话有上下文（1.0+方式）

> **重要**: LangChain 1.0+ 中，`ConversationChain` 和 `ConversationBufferMemory` 已废弃。推荐使用 `RunnableWithMessageHistory` + `ChatMessageHistory`。
...
# 创建Prompt模板（1.0+ 使用 MessagesPlaceholder 插入历史消息）
prompt = ChatPromptTemplate.from_messages([
    ("system", "你是一个有帮助的助手，记住用户说的每句话。"),
...
print(response2.content)  # 应该提到"亮亮"和"LLM开发"
```
...
| 类型 | 说明 | 适用场景 |
|------|------|----------|
| ChatMessageHistory | 保存完整对话历史（基础） | 短对话 |
| RunnableWithMessageHistory | 包装任意链添加记忆 | 通用场景 |
| InMemoryChatMessageHistory | 内存存储 | 开发测试 |
| RedisChatMessageHistory | Redis存储 | 生产环境 |
| PostgresChatMessageHistory | PostgreSQL存储 | 生产环境 |

### 2.5 OutputParser（输出解析器）（1.0+方式）

让模型输出结构化的数据（如JSON）：

> **1.0+ 变化**: 使用 `JsonOutputParser` 替代 `StructuredOutputParser`，配合 `PydanticOutputParser` 可以更严格地约束输出格式。
...
result = chain.invoke({"text": "今天终于完成了项目！"})
print(result)
```

### 2.6 综合示例：带记忆的问答系统（1.0+完整版）
...
# 多轮对话示例
config = {"configurable": {"session_id": "tech_qa"}}
...
print("Q2:", response2.content)

# 第三轮
response3 = conversation.invoke(
    {"question": "这两种方式各适合什么场景？"},
    config=config
)
print("Q3:", response3.content)

# 查看完整对话历史
history = store["tech_qa"]
print("\n完整历史：")
